...
def get_movie(movie_id: int, tmdb_token: str):
    if not tmdb_token:
        raise HTTPException(status_code=401, detail="No TMDB Key provided")
    url = f"{BASE_URL}/movie/{movie_id}"
    headers = {"Authorization": f"Bearer {tmdb_token}"}
    resp = requests.get(url, headers=headers)
    if resp.status_code != 200:
        raise HTTPException(status_code=resp.status_code, detail=f"TMDB SAYS: {resp.text}")
    return resp.json()
...
def get_show_image(movie_id: int, tmdb_token: str, trakt_token: str, trakt_client_id: str):
    ...
    if resp.status_code != 200:
        ...
        if trakt:
            return trakt
        raise HTTPException(status_code=resp.status_code, detail=f"{resp.text}. Trying a different Endpoint")
    return resp.json()
